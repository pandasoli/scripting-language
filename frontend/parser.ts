...
import { Stmt, Program, VarDeclaration, Expr, BinaryExpr, NumericLiteral, Identifier, AssignmentExpr, Property, ObjectLiteral, CallExpr, MemberExpr, FunctionDeclaration } from './ast.ts'
	...
	*/
	...
	private parse_assignment_expr(): Expr {
		...
		if (this.at().type == TokenType.Equals) {
			this.eat()
			const value = this.parse_assignment_expr() // allow: `x = foo = bar`
	...
	private parse_args(): Expr[] {
		this.expect(TokenType.OpenParen, 'Expected open parenthesis') // never gonna throw any error
	...
	// it uses parse_assignment_expr to allow: `foo(x = 5)` sending `5` to `foo`.
	...
	private parse_member_expr(): Expr {
		...
		while (this.at().type == TokenType.Dot || this.at().type == TokenType.OpenBracket) {
			...
			if (operator.type == TokenType.Dot) {
				...
				this.expect(TokenType.CloseBracket, 'Missing closing brakcet in computed value')
			}
